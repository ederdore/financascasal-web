...
import { BancoLogo } from '../components/BancoInfo.jsx'

const COLORS = ['#1D9E75','#178DD1','#EF9F27','#E24B4A','#7F77DD','#2E7D32','#993556','#FF6B35','#4ECDC4']

// ── Dash de compromissos futuros ──────────────────────
function CompromissosFuturos({ parcelas, recorrencias, cartao }) {
  if (!parcelas.length && !recorrencias.length) return null

  const now = new Date()
  const meses = []

  // Projeta 6 meses à frente
  for (let i = 0; i < 6; i++) {
    ...
    const parcsDoMes = parcelas.filter(p => {
      const restam = p.total_parcelas - p.parcela_atual
      if (restam <= 0) return false
      // Verifica se ainda terá parcela neste mês
      const mesInicio = new Date(p.created_at)
      const mesesDecorridos = (a - mesInicio.getFullYear()) * 12 + (m - mesInicio.getMonth())
      return mesesDecorridos >= 0 && mesesDecorridos < restam
    })

    // Recorrências (sempre presentes)
    const totalParcs = parcsDoMes.reduce((s, p) => s + p.valor_parcela, 0)
    const totalRecs  = recorrencias.reduce((s, r) => s + r.valor, 0)
    const total      = totalParcs + totalRecs
    const pct        = cartao.limite > 0 ? Math.min(100, (total / cartao.limite) * 100) : 0

    meses.push({
      label: `${MESES[m].substring(0,3)}/${String(a).slice(2)}`,
      mes: m, ano: a,
      totalParcs, totalRecs, total, pct,
      parcelas: parcsDoMes,
      atual: i === 0,
    })
  }
  ...
  return (
    <div className="card" style={{ marginBottom:16 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
        <div>
          <div style={{ fontWeight:600, fontSize:14 }}>📅 Compromissos futuros</div>
          <div style={{ fontSize:12, color:'var(--secondary)', marginTop:2 }}>
            Quanto já está comprometido nos próximos meses
          </div>
        </div>
        <div style={{ fontSize:11, color:'var(--secondary)', textAlign:'right' }}>
          <div style={{ display:'flex', gap:10 }}>
            <span style={{ display:'flex', alignItems:'center', gap:4 }}>
              <div style={{ width:8, height:8, borderRadius:2, background:'var(--yellow)' }}/>Parcelas
            </span>
            <span style={{ display:'flex', alignItems:'center', gap:4 }}>
              <div style={{ width:8, height:8, borderRadius:2, background:'var(--blue)' }}/>Assinaturas
            </span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:8, alignItems:'flex-end', marginBottom:16 }}>
        {meses.map((m, i) => (
          <div key={i} style={{ flex:1, textAlign:'center' }}>
            <div style={{ fontSize:11, fontWeight:600, color: m.pct>80?'var(--red)':m.pct>50?'var(--yellow)':'var(--secondary)', marginBottom:4 }}>
              {fmt(m.total)}
            </div>
            <div style={{ height:80, display:'flex', flexDirection:'column', justifyContent:'flex-end', gap:1 }}>
              {/* Barra parcelas */}
              {m.totalParcs > 0 && (
                <div style={{
                  height: `${Math.max(4, (m.totalParcs/maxTotal)*70)}px`,
                  background:'var(--yellow)', borderRadius:'3px 3px 0 0', opacity: m.atual?1:0.7,
              ...
              {m.totalRecs > 0 && (
                <div style={{
                  height: `${Math.max(4, (m.totalRecs/maxTotal)*70)}px`,
                  background:'var(--blue)', borderRadius: m.totalParcs>0?'0':'3px 3px 0 0', opacity: m.atual?1:0.7,
                  transition:'height .4s'
                }}/>
              )}
            </div>
            <div style={{ fontSize:10, color: m.atual?'var(--primary)':'var(--secondary)', marginTop:6, fontWeight:m.atual?700:400 }}>
              {m.label}
            </div>
            <div style={{ fontSize:10, color: m.pct>80?'var(--red)':m.pct>50?'var(--yellow)':'var(--secondary)' }}>
              {m.pct.toFixed(0)}%
            </div>
          </div>
        ))}
      ...
      <div style={{ borderTop:'0.5px solid var(--border)', paddingTop:12 }}>
        <div style={{ fontSize:12, fontWeight:600, color:'var(--secondary)', textTransform:'uppercase', letterSpacing:0.5, marginBottom:8 }}>
          Detalhamento — {MESES[now.getMonth()]}
        </div>
            ...
            return (
              <div key={p.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:13 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  <span style={{ fontSize:11, color:'var(--secondary)' }}>({p.parcela_atual}/{p.total_parcelas})</span>
                </div>
                <div style={{ textAlign:'right' }}>
                  <span style={{ fontWeight:600, color:'var(--yellow)' }}>{fmt(p.valor_parcela)}/mês</span>
                  <div style={{ fontSize:10, color:'var(--secondary)' }}>por mais {restam} mes{restam>1?'es':''} · total {fmt(restam*p.valor_parcela)}</div>
                </div>
              </div>
            )
          })}
          {recorrencias.map(r => (
            <div key={r.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:13 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <span>{r.nome}</span>
                <span style={{ fontSize:11, color:'var(--secondary)' }}>recorrente</span>
              </div>
              <span style={{ fontWeight:600, color:'var(--blue)' }}>{fmt(r.valor)}/mês</span>
            </div>
          ))}
        </div>
        {(parcelas.length > 0 || recorrencias.length > 0) && (
          <div style={{ borderTop:'0.5px solid var(--border)', marginTop:10, paddingTop:10, display:'flex', justifyContent:'space-between', fontSize:13 }}>
            <span style={{ color:'var(--secondary)' }}>Total comprometido/mês</span>
            <span style={{ fontWeight:700, color: (meses[0]?.pct||0)>80?'var(--red)':(meses[0]?.pct||0)>50?'var(--yellow)':'var(--primary)' }}>
              {fmt(meses[0]?.total||0)}
            </span>
          </div>
        )}
...
export default function FaturaDetalhe({ session, profile, cartao, onVoltar }) {
  ...
  async function loadData() {
    ...
    const [d, p, r] = await Promise.all([
      cf(supabase.from('despesas').select('*')).eq('cartao_id', cartao.id).eq('mes', mes).eq('ano', ano).order('created_at', { ascending: false }),
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <div className="row-between">
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <BancoLogo nome={cartao.nome} size={40} />
            <div>
      ...
      <div className="grid-4" style={{ marginBottom:20 }}>
        ...
        <div className="mini-card">
          <div className="lbl">Parcelas/mês</div>
      ...
      <CompromissosFuturos parcelas={parcelas} recorrencias={recorrencias} cartao={cartao} />
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>Descrição</th><th>Categoria</th><th>Quem</th><th>Tipo</th><th>Valor</th></tr></thead>
            <tbody>
              {despesas.map(d => (
                <tr key={d.id}>
                  ...
                  <td><span className={`badge ${d.tipo==='recorrente'?'badge-green':d.tipo==='fixa'?'badge-blue':'badge-yellow'}`}>{d.tipo==='recorrente'?'🔄 Rec.':d.tipo==='fixa'?'📌 Fixa':'📦 Var.'}</span></td>
