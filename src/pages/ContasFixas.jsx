...
export default function ContasFixas({ session, profile }) {
  ...
  return (
    <div>
        ...
        <div style={{ background:'#FCEBEB', border:'0.5px solid var(--red)', borderRadius:12, padding:14, marginBottom:16 }}>
          ...
          {atrasadas.map(f => (
            <div key={f.id} className="row-between" style={{ padding:'6px 0', borderTop:'0.5px solid #FCCACA' }}>
              <div><span style={{ fontWeight:500 }}>{f.nome}</span><span style={{ fontSize:12, color:'var(--secondary)', marginLeft:8 }}>Venceu dia {f.dia_vencimento}</span></div>
      ...
      <div className="grid-3" style={{ marginBottom:20 }}>
        <div className="mini-card"><div className="lbl">Fixas/mês</div><div className="val" style={{ color:'var(--red)' }}>{fmt(totalFixas)}</div><div className="sub">{pagas}/{fixas.length} pagas</div></div>
        <div className="mini-card"><div className="lbl">Variáveis/mês</div><div className="val" style={{ color:'var(--yellow)' }}>~{fmt(totalVars)}</div><div className="sub">{vars.length} item(s)</div></div>
        <div className="mini-card"><div className="lbl">Total estimado</div><div className="val">{fmt(totalFixas + totalVars)}</div><div className="sub">fixas + variáveis</div></div>
      </div>
      ...
      <div className="card" style={{ marginBottom:20 }}>
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>Nome</th><th>Categoria</th><th>Responsável</th><th>Banco</th><th>Vence</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead>
            <tbody>
              {fixas.map(f => {
                  ...
                  <tr key={f.id} style={{ background: status==='atrasada'?'#FFF5F5':status==='paga'?'#F0FFF4':'transparent' }}>
                    ...
                    <td>
                      <div className="row" style={{ gap:6 }}>
                        {status!=='paga'?(<button className="btn btn-sm" style={{ background:'#E1F5EE', color:'var(--green)' }} onClick={() => openPagar(f)}>✅ Pagar</button>):(<button className="btn btn-outline btn-sm" onClick={() => marcarPaga(f.id, false)}>↩️</button>)}
                        <button className="btn btn-outline btn-sm" onClick={() => openFixa(f)}>✏️</button>
                        <button className="btn btn-sm" style={{ background:'#FCEBEB', color:'var(--red)' }} onClick={async()=>{if(confirm('Excluir?')){await supabase.from('contas_fixas').delete().eq('id',f.id);loadData()}}}>🗑️</button>
                      </div>
                    </td>
                  </tr>
                )
                ...
                <tr key={v.id}>
                  ...
                  <td>
                    <div className="row" style={{ gap:6 }}>
                      ...
                      <button className="btn btn-sm" style={{ background:'#FCEBEB', color:'var(--red)' }} onClick={async()=>{if(confirm('Excluir?')){await supabase.from('contas_variaveis').delete().eq('id',v.id);loadData()}}}>🗑️</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="modal-overlay" onClick={() => setModalPagar(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={confirmarPagamento}>
              <div className="form-group"><label className="form-label">Valor pago (R$)</label><input className="form-input" type="number" step="0.01" value={pagValor} onChange={e => setPagValor(e.target.value)} required /></div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={salvarFixa}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                <div className="form-group"><label className="form-label">Categoria</label><select className="form-select" value={fxCat} onChange={e => setFxCat(e.target.value)}>{(CATS_FIXA||['Moradia','Saúde','Educação','Transporte','Outros']).map(c=><option key={c}>{c}</option>)}</select></div>
              ...
              <div className="form-group"><label className="form-label">Banco de pagamento</label>
        ...
        <div className="modal-overlay" onClick={() => setModalVar(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={salvarVar}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                <div className="form-group"><label className="form-label">Categoria</label><select className="form-select" value={vrCat} onChange={e => setVrCat(e.target.value)}>{(CATS_VAR||['Alimentação','Lazer','Transporte','Saúde','Outros']).map(c=><option key={c}>{c}</option>)}</select></div>
