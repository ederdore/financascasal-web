...
export default function Despesas({ session, profile }) {
  ...
  const [aba, setAba] = useState('lista') // lista | graficos
  ...
  async function salvar(e) {
    ...
    try {
      if (pagTipo === 'cartao' && cartaoId && parcelado && parseInt(nParcelas) > 1) {
        ...
        const payload = cp({ nome, valor: v, categoria: cat, quem, tipo: tipoFinal, mes: now.getMonth(), ano: now.getFullYear(), pagamento_tipo: pagTipo, cartao_id: pagTipo === 'cartao' && cartaoId ? cartaoId : null, cartao_nome: pagTipo === 'cartao' ? cartaoNome : '', banco_id: pagTipo === 'debito' && bancoId ? bancoId : null, banco_nome: pagTipo === 'debito' ? bancoNome : '' })
  ...
  // Dados gráfico pizza — categorias
  const catMap = {}
  ...
  // Dados gráfico barras — por mês (últimos 6 meses)
  const barData = []
  for (let i = 5; i >= 0; i--) {
    ...
    barData.push({
      mes: MESES_CURTO[m],
      Variável: desp.filter(x => x.tipo === 'variavel').reduce((s, x) => s + x.valor, 0),
      Fixa: desp.filter(x => x.tipo === 'fixa').reduce((s, x) => s + x.valor, 0),
      Recorrente: desp.filter(x => x.tipo === 'recorrente').reduce((s, x) => s + x.valor, 0),
    })
  }
  ...
  const tipoData = [
    { name: 'Variável', value: tipoMap.variavel },
    { name: 'Fixa', value: tipoMap.fixa },
    { name: 'Recorrente', value: tipoMap.recorrente },
  ].filter(x => x.value > 0)
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
        ...
        <select className="form-select" style={{ width: 'auto' }} value={filtroCat} onChange={e => setFiltroCat(e.target.value)}>
        ...
        {(filtroCat || filtroTipo || filtroQuem) && (
          <button className="btn btn-outline btn-sm" onClick={() => { setFiltroCat(''); setFiltroTipo(''); setFiltroQuem('') }}>✕ Limpar</button>
        )}
      </div>

      {/* Abas */}
      <div style={{ display: 'flex', gap: 0, borderBottom: '0.5px solid var(--border)', marginBottom: 16 }}>
        {[['lista','📋 Lista'], ['graficos','📊 Gráficos']].map(([id, label]) => (
      ...
      {aba === 'lista' && (
        <div className="card">
          <div className="table-wrap">
            <table>
              <thead><tr><th>Descrição</th><th>Categoria</th><th>Quem</th><th>Pagamento</th><th>Tipo</th><th>Mês</th><th>Valor</th><th>Ações</th></tr></thead>
              <tbody>
                {despFiltradas.map(d => (
                  <tr key={d.id}>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={salvar}>
              <div className="form-group"><label className="form-label">Descrição</label>
                <input className="form-input" placeholder="Ex: Supermercado..." value={nome} onChange={e => setNome(e.target.value)} required /></div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div className="form-group"><label className="form-label">Quem pagou?</label>
              ...
              {pagTipo === 'debito' && (
                <div className="form-group"><label className="form-label">Banco</label>
              ...
              {pagTipo === 'cartao' && (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div className="form-group"><label className="form-label">Parcelado?</label>
              ...
              <form onSubmit={adicionarCat}>
                <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr', gap: 10 }}>
                  <div className="form-group"><label className="form-label">Ícone</label>
                    <input className="form-input" value={novaCatIcone} onChange={e => setNovaCatIcone(e.target.value)} placeholder="📦" style={{ textAlign: 'center', fontSize: 20 }} /></div>
                  <div className="form-group"><label className="form-label">Nome da categoria</label>
                    <input className="form-input" placeholder="Ex: Animais, Farmácia..." value={novaCatNome} onChange={e => setNovaCatNome(e.target.value)} /></div>
                </div>
